...
export function inferNameForAutoGenerated(node: SceneNode): string | null {
  ...
  if ('children' in node) {
    ...
    if (children.length === 1) {
      ...
      // 자식이 의미 있는 이름이면 "Content" 반환
      if (!isAutoGeneratedName(child.name) && !isGenericName(child.name)) {
        return 'Content';
      }
    }
  ...
  if (parent && 'name' in parent) {
    ...
    // 부모가 Section/이면 Content
    if (parentName.startsWith('Section/')) {
      return 'Content';
    }

    // 부모가 Layout/이면 Content
...
const STRUCTURAL_LAYER_NAMES: Record<string, string> = {
  ...
  'ftr': 'Footer',
  'list': 'List',
  'container': 'Content',  // Container → Content로 변경 (더 일반적)
  'wrapper': 'Content',    // Wrapper → Content로 변경
  'nav': 'Navigation',
 ...
 * - 자식 개수/구조 기반으로 의미 있는 이름 추론
 * - 'Container'는 피하고 더 구체적인 이름 사용
 */
export function generateAlternativeName(node: SceneNode): string {
  // 자식이 있으면 구조 기반
  if ('children' in node) {
    const children = (node as FrameNode).children;

    // 자식이 1개인 경우 - 자식 타입에 따라 이름 추론
    if (children.length === 1) {
      const child = children[0];
      const childNameLower = child.name.toLowerCase();

      // 자식이 Icon이면 IconWrapper 대신 Content
      if (childNameLower.includes('icon') || child.name.startsWith('Icon/')) {
        return 'Content';
      }
      // 자식이 Text면 Content
      if (child.type === 'TEXT' || childNameLower.includes('text') || childNameLower.includes('label')) {
        return 'Content';
      }
      // 자식이 Image면 Content
      if (childNameLower.includes('image') || childNameLower.includes('img')) {
        return 'Content';
      }
      // 기본값: Content (Container 대신)
      return 'Content';
    }
...
const LAYOUT_KEYWORD_MAP: Record<string, string> = {
  ...
  'topbar': 'TopBar',
  'top bar': 'TopBar',
  'container_top': 'TopBar',
  'profile container': 'TopBar',
  'notification container': 'TopBar',

  // Main 관련 (content, body는 너무 일반적이라 제외)
  'main': 'Main',
  'main content': 'Main',
  'main container': 'Main',
  'main body': 'Main',
...
export function inferLayoutName(node: SceneNode): string | null {
  ...
  if (LAYOUT_KEYWORD_MAP[nameLower]) {
    return `Layout/${LAYOUT_KEYWORD_MAP[nameLower]}`;
  }
  ...
  // 'main'만 Layout/Main으로 (content, body는 너무 일반적이라 제외)
