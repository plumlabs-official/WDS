...
function hasShapeDescendant(node: SceneNode, depth: number = 0): boolean {
  ...
  var SHAPE_TYPES = ['RECTANGLE', 'ELLIPSE', 'POLYGON', 'STAR', 'LINE', 'VECTOR', 'BOOLEAN_OPERATION'];
  if (SHAPE_TYPES.indexOf(node.type) !== -1) {
    return true;
  }
...
export function isMeaninglessWrapper(node: SceneNode): boolean {
  ...
  if (node.type === 'GROUP') {
    ...
    var SHAPE_TYPES_GROUP = ['RECTANGLE', 'ELLIPSE', 'POLYGON', 'STAR', 'LINE', 'VECTOR', 'BOOLEAN_OPERATION'];
    if (SHAPE_TYPES_GROUP.indexOf(groupChild.type) !== -1) {
  ...
  if (node.type === 'FRAME') {
    ...
    // 자식이 shape 타입이면 래퍼가 아님 (시각적 컨테이너 역할)
    // Shape: RECTANGLE, ELLIPSE, POLYGON, STAR, LINE, VECTOR, BOOLEAN_OPERATION
    var SHAPE_TYPES = ['RECTANGLE', 'ELLIPSE', 'POLYGON', 'STAR', 'LINE', 'VECTOR', 'BOOLEAN_OPERATION'];
    if (SHAPE_TYPES.indexOf(frameChild.type) !== -1) {
...
export function isAggressiveWrapper(node: SceneNode): boolean {
  ...
  // shape 타입 자식은 보존
  var SHAPE_TYPES = ['RECTANGLE', 'ELLIPSE', 'POLYGON', 'STAR', 'LINE', 'VECTOR', 'BOOLEAN_OPERATION'];
  if (SHAPE_TYPES.indexOf(child.type) !== -1) {
    return false;
  }
...
function isEmptyImageFrame(node: SceneNode): boolean {
  ...
  function hasVisualContent(n: SceneNode): boolean {
    ...
    var SHAPE_TYPES = ['RECTANGLE', 'ELLIPSE', 'POLYGON', 'STAR'];
    if (SHAPE_TYPES.indexOf(n.type) !== -1) {
      // fills가 있으면 시각적 콘텐츠
      if ('fills' in n) {
