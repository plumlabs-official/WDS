 ...
 * 동일 이름 중첩 레이어 병합 핸들러
 */
function handleFlattenSameName() {
  figma.ui.postMessage({ type: 'task-start', taskName: '중첩 레이어 병합 중...' });

  try {
    const result = flattenSelectionSameNameWrappers();
...
async function handleNamingAgent() {
  ...
  for (const node of allNodes) {
    ...
    if (directName && directName !== node.name) {
      directChanges.push({
        node,
        oldName: node.name,
        newName: directName,
      });
    } else if (node.type === 'FRAME' && !directName) {
      // FRAME이고 직접 변환 불가능하면 AI 분석 대상
      // 유효한 시맨틱 타입이 아니면 재분석 (HomeScreen/... 같은 잘못된 패턴 포함)
      if (!hasValidSemanticName(node.name)) {
